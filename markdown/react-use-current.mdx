<h1 className='font-semibold'>React Use Current — A Tiny React Hook for Synchronous Reactive State</h1>
...
## 🚀 React Use Current — Reactive + Synchronous State in React
...
> **Direct mutation that triggers re-renders — immediately and safely.**

---

## 🔍 What Is React Use Current?
...
### ❌ "I need immediate state updates"

`useState` doesn't update until next render.  
`useCurrent` updates **instantly**.

### ❌ "I want to mutate state directly (nested objects)"

`useState` forces immutability → slow & annoying.  
`useCurrent` allows:
...
### ❌ "useRef doesn't re-render when changed"
...
| Synchronous | ❌ | ✔️ | ✔️ |
| Deep mutation | ❌ | ✔️ | ✔️ |
| Triggers re-render | ✔️ | ❌ | ✔️ |
| Requires immutable updates | ✔️ | ❌ | ❌ |
| Built on VRef | ❌ | ❌ | ✔️ |
...
- Works deeply — arrays, objects, nested structures.

---

## 🧩 Real Example — Objects, Deep Mutation, and Tracking
...
## 🧬 Understanding the `track()` Function
...
This mechanism is identical to VRef's change reporters — but adapted for React.
...
### ✔ Synchronous logic (counters, timers, scrolling)

Get immediate updates—no batching.

### ✔ Local reactive stores

Like lightweight MobX-style state.

### ✔ Avoid expensive cloning in large objects
...
👉 <a href="https://stackblitz.com/edit/react-use-current?file=src/App.tsx" target='_blank'>Open Live Playground</a>
</div>
...
🟢 npm: [https://www.npmjs.com/package/react-use-current](https://www.npmjs.com/package/react-use-current)  
🐙 GitHub: [https://github.com/JohnSoatra/react-use-current](https://github.com/JohnSoatra/react-use-current)  

Reference:  
🌍 Docs: [https://doc.soatra.com/vref](https://doc.soatra.com/vref) (VRef base)
